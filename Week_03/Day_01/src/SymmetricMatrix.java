...
public class SymmetricMatrix {
  ...
  static boolean isSymmetric(ArrayList<ArrayList<Integer>> matrix) {
    for (int i = 0; i < matrix.size(); i++) {
      if (matrix.size() != matrix.get(i).size()) {
        return false;
      }
      //for (int j = 0 .....) also god, but the one below is better because it's not gonna check anything twice.
